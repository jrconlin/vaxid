...
from vaxid.utils import b64urldecode, b64urlencode
from vaxid.jwt import sign

# Show compliance version. For earlier versions see previously tagged releases.
VERSION = "VAPID-RFC/ECE-RFC"


class VaxidException(Exception):
    """An exception wrapper for Vapid."""
    pass


class Vaxid(object):
    """Minimal Vaxid signature generation library.
    ...
    def from_raw(cls, private_raw):
        """Initialize Vaxid using a private key point in "raw" or
    ...
    def from_file(cls, private_key_file=None):
        """Initialize Vaxid using a file containing a private key in PEM or
        DER format.
        ...
        if not os.path.isfile(private_key_file):
            ...
            vaxid = cls()
            vaxid.generate_keys()
            vaxid.save_key(private_key_file)
            return vaxid
        with open(private_key_file, 'r') as file:
            private_key = file.read()
        try:
            if "-----BEGIN" in private_key:
                vaxid = cls.from_pem(private_key.encode('utf8'))
            else:
                vaxid = cls.from_der(private_key.encode('utf8'))
            return vaxid
        except Exception as exc:
            logging.error("Could not open private key file: %s", repr(exc))
            raise VaxidException(exc)

    @classmethod
    def from_string(cls, private_key):
        """Initialize Vaxid using a string containing the private key. This
    ...
    def private_key(self):
        """The Vaxid private ECDSA key"""
        if not self._private_key:
            raise VaxidException("No private key. Call generate_keys()")
        return self._private_key

    @private_key.setter
    def private_key(self, value):
        """Set the Vaxid private ECDSA key
    ...
    def sign(self, claims, crypto_key=None):
        sig = sign(self._claim_check(claims), self.private_key)
        pkey = self.public_key.public_bytes(
                serialization.Encoding.X962,
                serialization.PublicFormat.UncompressedPoint
            )
        return "{schema} t={t},k={k}".format(
                schema=self._schema,
                t=sig,
                k=b64urlencode(pkey)
            )
